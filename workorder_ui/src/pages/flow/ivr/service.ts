...
import { addIvrFlow,getIvrFlow } from './dataservice';
...
export namespace MockApi {
  ...
  export const loadGraphData = async (meta: NsGraph.IGraphMeta) => {
    ...
    console.log('-------------------meta=',meta.flowId)
    let id = parseInt(meta.flowId)
    const resp = await getIvrFlow(id);
    if(resp.code === 200) {
      const data: NsGraph.IGraphData=JSON.parse(resp.data.content) || { nodes: [], edges: [] }
      return data;
    } else {
  ...
  export const saveGraphData: NsGraphCmd.SaveGraphData.IArgs['saveGraphDataService'] = async (
    ...
    console.log('saveGraphData json', JSON.stringify(graphData))
    const flowId = parseInt(meta.flowId)
  ...
  export const addNode: NsNodeCmd.AddNode.IArgs['createNodeService'] = async (
    ...
    const { id,ports, groupChildren } = args.nodeConfig
