...
import {IProps} from "@/pages/flow/ivr/index";
...
export namespace NSToolbarConfig {
  ...
  export const getToolbarItems = async (state: IToolbarState,name: string) => {
    ...
    toolbarGroup4.push({
      id: XFlowDagCommands.QUERY_GRAPH_STATUS.id + 'title',
      text: name,
      tooltip: name,
    })
    return [
      ...
      {name: 'customCmd',items: toolbarGroup3},
      {name: 'customTitle',items: toolbarGroup4},
    ]
  }
}
export const useToolbarConfig = createToolbarConfig<IProps>((toolbarConfig, proxy) => {
  /** 生产 toolbar item */
  ...
  toolbarConfig.setToolbarModelService(async (toolbarModel, modelService, toDispose) => {
    const updateToolbarModel = async () => {
      ...
      const toolbarItems = await NSToolbarConfig.getToolbarItems(state,props.meta.name)
