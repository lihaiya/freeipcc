...
public class MailSender {
	...
	public MailSender() {
		init("rts@dbappsecurity.com.cn", "anheng@123", "smtp.dbappsecurity.com.cn","","");
	}
...
//		message.setSubject(subject);
	 ...
	 * @throws MessagingException
	 */
	public void send(List<String> recipients,List<String> recipientAppends, String subject, Object content,List<String> filenames)
			throws Exception {
		// 创建mime类型邮件
		final MimeMessage message = new MimeMessage(session);
		if(this.fromEmail!=null){
		...
		final int ccnum = recipientAppends.size();
		InternetAddress[] ccaddresses = new InternetAddress[ccnum];
		for (int i = 0; i < ccnum; i++) {
			ccaddresses[i] = new InternetAddress(recipientAppends.get(i));
		}
